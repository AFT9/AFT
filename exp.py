from aft import AFT
from FairnessTestMethods import Vbtx, Vbt, ExpGA, Themis
...
def exp(dataset_name, model_name, protected_pair, method, runtime, repeat, show_logging, res_logging=False, repeat_label=None, start_label=0):
    ...
    for _ in range(start_label, start_label + repeat):
        ...
        if method == "aft":
            ...
            threshold_l = {"Adult": 7, "Credit": 14, "Bank": 10}[dataset_name]
            original_data = df.iloc[:, :-1].values.astype(float)
            tester = ExpGA(CuT, [protected_param], protected_name=protected_name,
                           threshold_l=threshold_l, original_data=original_data,
                           show_logging=show_logging)
            tester.xai_fair_testing(runtime=runtime, seed_num=None,
                        label=(f"{method}-{model_name}-{dataset_name}-{protected_name}-{runtime}", _))
        ...
        if res_logging:
            option_reports = ""
            if method in ["expga"]:
                option_reports = f",{tester.local_cpu_time_consumed},{tester.local_real_time_consumed}"
...
def para_exp_main(runtime, repeat, repeat_run_together=True):
    ...
    method_list = ["aft", "vbtx", "vbt", "themis", "expga"]
    start_label = 0
    paras = list()
...
if __name__ == "__main__":
    ...
    parser.add_argument("--method", choices=["aft", "vbtx", "vbt", "expga", "themis"],
